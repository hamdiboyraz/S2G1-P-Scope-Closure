...
function macSonucu(score, quarter) {
  const scores = {
    EvSahibi: 0,
    KonukTakim: 0,
  };
  ...
  for (let i = 0; i < quarter; i++) {
    scores.EvSahibi += score();
    scores.KonukTakim += score();
  }

  while (scores.EvSahibi === scores.KonukTakim) {
    scores.EvSahibi += score();
    scores.KonukTakim += score();
  }
...
function skorTabelasi(periodScore, score, quarter) {
  let homeTeam = 0;
  let awayTeam = 0;
  const scores = [];
  const table = [];
  ...
  for (let i = 0; i < quarter; i++) {
    scores.push(periodScore(score));
    console.log(scores);
    homeTeam += scores[i].EvSahibi;
    awayTeam += scores[i].KonukTakim;
    table.push(
      `${i + 1}. Periyot: Ev Sahibi ${scores[i].EvSahibi} - Konuk Takım ${
        scores[i].KonukTakim
      }`
    );
  }
  ...
  while (homeTeam === awayTeam) {
    scores.push(periodScore(score));
    homeTeam += scores[scores.length - 1].EvSahibi;
    awayTeam += scores[scores.length - 1].KonukTakim;
    table.push(
      `${i + 1}. Uzatma: Ev Sahibi ${scores[i].EvSahibi} - Konuk Takım ${
        scores[i].KonukTakim
      }`
    );
  }

  table.push(`Maç Sonucu: Ev Sahibi ${homeTeam} - Konuk Takım ${awayTeam}`);
  return table;
}
